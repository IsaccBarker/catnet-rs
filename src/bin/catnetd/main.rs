...
fn main() {
    let matches = App::new(env!("CARGO_PKG_NAME"))
        ...
        .arg(
            Arg::from("-l, --logging 'Sets a logging level to display.'")
                .required(false)
                .min_values(0)
        ...
        .subcommand(
            App::new("registrar")
                ...
                .arg(
                    Arg::from("<ip> -i, --ip 'The ip to host on.'")
                        .default_value("127.0.0.1")
                        .required(false),
                )
                .arg(
                    Arg::from("<port> -p, --port, 'The port to listen on.'")
                        .default_value("44400")
                        .required(false),
                ),
        )
        .subcommand(
            App::new("participant")
                ...
                .arg(
                    Arg::from("<ip> -i, --ip 'The ip that the registrar exists on.'")
                        .default_value("127.0.0.1")
                        .required(false),
                )
                .arg(
                    Arg::from("<port> -p, --port 'The port that registrar exists on.'")
                        .default_value("44400")
                        .required(false),
                ),
        )
    ...
    match matches.subcommand() {
        Some(("registrar", registrar_matches)) => {
            ...
            let ip = registrar_matches
                .value_of("ip")
                .expect("Expected a default value!");
            let port = registrar_matches
                .value_of("port")
                .expect("Expected a default value!");
        ...
        Some(("participant", participant_matches)) => {
            ...
            let ip = participant_matches
                .value_of("ip")
                .expect("Expected a default value!");
            let port = participant_matches
                .value_of("port")
                .expect("Expected a defaut value!");
            let addr = ip.to_owned() + ":" + port;
